...
(async () => {
  ...
  try {
    ...
    // --- The running app polishes every imported track ON ARRIVAL, so the rendered
    // product itself shows the post-processing state without a button hunt. Wait for
    // all three to reach Polished. ---
    ...
    check(completeCount === 3, `All three imported tracks polished on arrival (saw ${completeCount})`);
    ...
    check(await page.locator("#audio-apply-btn").isVisible(), "Apply audio & continue stays visible as the commit action");

    // --- CLICK Apply audio & continue — commit the saved polished assets into the
    // step and advance (the exact action whose outcome the visual probe never saw). ---
    ...
    check(true, "Apply audio & continue committed the polished tracks and advanced");
    ...
    check(/\d+\/\d+ tracks? polished/.test(reviewText), "Publish review consumes the polished tracks (shows N/N polished)");
  } catch (err) {
