  ...
  function runChecks(episodeSummary, ctx) {
    ...
    if (audioState.ready) {
      checks.push(check(
        ...
        "ok",
        "Audio polished",
        `${context.audioPolish.presetName} · ${context.audioPolish.treatmentLine || "treatment applied"} · ${audioState.treated}/${audioState.total} track${audioState.total === 1 ? "" : "s"} polished`,
        null,
      ));
    } else {
      checks.push(check(
      ...
      },
      {
        ...
        summary: (function () {
          ...
          return state.treated
            ? `${a.presetName} · ${state.treated}/${state.total} track${state.total === 1 ? "" : "s"} polished`
            : a.presetName;
        }()),
