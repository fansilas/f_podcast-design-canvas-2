...
(function () {
  ...
  function finishAudioProcessing(summary, ui) {
    ...
    if (AP.allTracksComplete(audioPolish)) {
      ...
      ui.progressLine.textContent = `All ${AP.processableCount(audioPolish)} tracks polished — saved as durable assets review and export render from.`;
    } else {
  ...
  function runAudioProcessing(summary, ui) {
    ...
    const step = (i) => {
      if (i >= tracks.length) { finishAudioProcessing(summary, ui); return; }
  ...
  function renderAudioPolish(summary) {
    ...
    tracksCard.appendChild(
      el("p", { class: "hint" }, "Apply audio decodes each imported track and saves a polished version that review and export use. Supported uploads: WAV, MP3, M4A/AAC, MP4, WebM."),
    );
    ...
    const progressLine = el("p", { class: "audio-progress-line", id: "audio-progress-line" },
      processable === 0
        ...
        : (allDone
          ? `All ${processable} tracks polished — saved as durable assets review and export render from.`
    ...
    applyButton.style.display = allDone ? "none" : "";
    continueButton.style.display = allDone ? "" : "none";
    ...
    const actionHint = el("p", { class: "audio-action-hint" },
      processable === 0
        ...
        : `Click Apply to process ${processable} imported track${processable === 1 ? "" : "s"} into polished audio.`);
